...
public final class FileSystemModule: Module {
  ...
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("getInfoAsync") { (url: URL, options: InfoOptions, promise: Promise) in
      switch url.scheme {
      case "file":
        EXFileSystemLocalFileHandler.getInfoForFile(url, resolver: promise.resolver, rejecter: promise.legacyRejecter)
      case "assets-library", "ph":
        EXFileSystemAssetLibraryHandler.getInfoForFile(url, resolver: promise.resolver, rejecter: promise.legacyRejecter)
      default:
    ...
    AsyncFunction("uploadTaskStartAsync") { (targetUrl: URL, localUrl: URL, uuid: String, options: UploadOptions, promise: Promise) in
      let session = options.sessionType == .background ? backgroundSession : foregroundSession
      let task = createUploadTask(session: session, targetUrl: targetUrl, sourceUrl: localUrl, options: options)
      let onSend: EXUploadDelegateOnSendCallback = { [weak self] task, bytesSent, totalBytesSent, totalBytesExpectedToSend in
    ...
    AsyncFunction("downloadResumableStartAsync") { (sourceUrl: URL, localUrl: URL, uuid: String, options: DownloadOptions, resumeDataString: String?, promise: Promise) in
      ...
      let onWrite: EXDownloadDelegateOnWriteCallback = { [weak self] task, bytesWritten, totalBytesWritten, totalBytesExpectedToWrite in
  ...
  private func ensurePathPermission(path: String, flag: EXFileSystemPermissionFlags) throws {
    guard let permissionsManager: EXFilePermissionModuleInterface = appContext?.legacyModule(implementing: EXFilePermissionModuleInterface.self),
          permissionsManager.getPathPermissions(path).contains(flag) else {
